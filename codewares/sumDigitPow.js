function sumDigPow(start, stop) {
  let arr = [...Array(stop - start + 1)].map((each, index) => each = index + start);
  let arrNew = arr.filter((each) =>
    each === [...String(each)].reduce((a, b, i) => (a + Number(b) ** (i + 1)), 0)
  )
  return arrNew;
}
...
// that may receive two integers a, b 
// that defines the range [a, b] (inclusive) 
// and outputs a list of the sorted numbers in the range 
