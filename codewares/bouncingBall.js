...
// bounce > 0 && bounce < 1
// window < h 
// seeBallInWindow <=  hCurrent > window
// result => integer Number / -1 (condition isn't fullfilled)

function bouncingBall(h, bounce, window) {
  if (!(h > 0) || !(bounce > 0 && bounce < 1) || !(window < h)) return -1;
  if (h < window) return 0;
  let sum = 1;
  function seeIt(hCurrent) {
    if (hCurrent > window) {
      sum += 2;
      return (seeIt(hCurrent * bounce));
    }
    return sum;
  }
  return seeIt(h * bounce);
}

console.log(bouncingBall(30.0, 0.9, 1.5));
